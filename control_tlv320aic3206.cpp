...
bool AudioControlTLV320AIC3206::inputSelect(int n) {
    if (n == AIC3206_INPUT_IN1) {
        aic_writeAddress(AIC3206_MICPGA_LEFT_POSITIVE_REG,
                         AIC3206_MIC_ROUTING_POSITIVE_IN1 & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        ...
        aic_writeAddress(AIC3206_MICPGA_RIGHT_POSITIVE_REG,
                         AIC3206_MIC_ROUTING_POSITIVE_IN1 & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        aic_writeAddress(AIC3206_MICPGA_RIGHT_NEGATIVE_REG,
                         AIC3206_MIC_ROUTING_NEGATIVE_CM_TO_CM1L & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        // BIAS OFF
        setMicBias(AIC3206_MIC_BIAS_OFF);

        if (debugToSerial) {
            Serial.println("Set Audio Input to IN1");
        }
        return true;
    } else if (n == AIC3206_INPUT_IN3_MICBIAS) {
        aic_writeAddress(AIC3206_MICPGA_LEFT_POSITIVE_REG,
                         AIC3206_MIC_ROUTING_POSITIVE_IN3 & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        aic_writeAddress(AIC3206_MICPGA_LEFT_NEGATIVE_REG,
                         AIC3206_MIC_ROUTING_NEGATIVE_CM_TO_CM1L & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        aic_writeAddress(AIC3206_MICPGA_RIGHT_POSITIVE_REG,
                         AIC3206_MIC_ROUTING_POSITIVE_IN3 & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        ...
        if (debugToSerial) {
            Serial.println("Set Audio Input to IN3, BIAS SET TO DEFAULT 2.5V");
        }
        return true;
        ...
        if (debugToSerial) {
            Serial.println("Set Audio Input to IN3, BIAS OFF");
        }
        return true;
    } else if (n == AIC3206_INPUT_IN2) {
        aic_writeAddress(AIC3206_MICPGA_LEFT_POSITIVE_REG,
                         AIC3206_MIC_ROUTING_POSITIVE_IN2 & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        aic_writeAddress(AIC3206_MICPGA_LEFT_NEGATIVE_REG,
                         AIC3206_MIC_ROUTING_NEGATIVE_CM_TO_CM1L & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        aic_writeAddress(AIC3206_MICPGA_RIGHT_POSITIVE_REG,
                         AIC3206_MIC_ROUTING_POSITIVE_IN2 & AIC3206_MIC_ROUTING_RESISTANCE_DEFAULT);
        ...
        setMicBias(AIC3206_MIC_BIAS_OFF);
        if (debugToSerial) {
            Serial.println("Set Audio Input to IN2, BIAS OFF");
        }
...
bool AudioControlTLV320AIC3206::setMicBias(int n) {
    if (n == AIC3206_MIC_BIAS_1_25) {
        aic_writeAddress(AIC3206_MIC_BIAS_REG,
                         AIC3206_MIC_BIAS_POWER_ON | AIC3206_MIC_BIAS_OUTPUT_VOLTAGE_1_25); // power up mic bias
        return true;
    } else if (n == AIC3206_MIC_BIAS_1_7) {
        aic_writeAddress(AIC3206_MIC_BIAS_REG,
                         AIC3206_MIC_BIAS_POWER_ON | AIC3206_MIC_BIAS_OUTPUT_VOLTAGE_1_7); // power up mic bias
        return true;
    } else if (n == AIC3206_MIC_BIAS_2_5) {
        aic_writeAddress(AIC3206_MIC_BIAS_REG,
                         AIC3206_MIC_BIAS_POWER_ON | AIC3206_MIC_BIAS_OUTPUT_VOLTAGE_2_5); // power up mic bias
        return true;
    } else if (n == AIC3206_MIC_BIAS_VSUPPLY) {
        aic_writeAddress(AIC3206_MIC_BIAS_REG,
                         AIC3206_MIC_BIAS_POWER_ON | AIC3206_MIC_BIAS_OUTPUT_VOLTAGE_VSUPPLY); // power up mic bias
        return true;
...
void AudioControlTLV320AIC3206::aic_initADC() {
    ...
    aic_writePage(1, 61,
                  0); // 0x3D // Select ADC PTM_R4 Power Tune?  (this line is from datasheet (application guide, Section 4.2)
    ...
    aic_writeAddress(AIC3206_MICPGA_RIGHT_VOLUME_REG,
                     AIC3206_MICPGA_VOLUME_ENABLE); // enable Right MicPGA, set gain to 0 dB

    aic_writeAddress(AIC3206_ADC_MUTE_REG, AIC3206_ADC_UNMUTE); // Unmute Left and Right ADC Digital Volume Control
    aic_writeAddress(AIC3206_ADC_CHANNEL_POWER_REG,
                     AIC3206_ADC_CHANNELS_ON); // Unmute Left and Right ADC Digital Volume Control
}
...
bool AudioControlTLV320AIC3206::setInputGain_dB(float volume) {
    ...
    aic_writeAddress(AIC3206_MICPGA_LEFT_VOLUME_REG,
                     AIC3206_MICPGA_VOLUME_ENABLE | volume_int); // enable Left MicPGA, set gain to 0 dB
    aic_writeAddress(AIC3206_MICPGA_RIGHT_VOLUME_REG,
                     AIC3206_MICPGA_VOLUME_ENABLE | volume_int); // enable Right MicPGA, set gain to 0 dB
    return true;
}
...
void AudioControlTLV320AIC3206::aic_initDAC() {
    if (debugToSerial) {
        Serial.println("controlTLV320AIC3206: Initializing AIC DAC");
    }
...
bool AudioControlTLV320AIC3206::outputSelect(int n) {
    ...
    if (n == AIC3206_OUTPUT_HEADPHONE_JACK_OUT) {
        ...
        if (debugToSerial) {
            Serial.println("controlTLV320AIC3206: Set Audio Output to Headphone Jack");
        }
        return true;
        ...
        if (debugToSerial) {
            Serial.println("controlTLV320AIC3206: Set Audio Output to Line Out");
        }
        return true;
        ...
        if (debugToSerial) {
            Serial.println("controlTLV320AIC3206: Set Audio Output to Headphone Jack and Line out");
        }
        return true;
    }
...
void AudioControlTLV320AIC3206::aic_init() {
    if (debugToSerial) {
        Serial.println("controlTLV320AIC3206: Initializing AIC");
    }
    ...
    // POWER
    aic_writePage(1, 0x01,
                  8); // Reg 1, Val = 8 = 0b00001000 = disable weak connection AVDD to DVDD.  Keep headphone charge pump disabled.
    aic_writePage(1, 0x02, 0); // Reg 2, Val = 0 = 0b00000000 = Enable Master Analog Power Control
    aic_writePage(1, 0x7B,
                  1); // Reg 123, Val = 1 = 0b00000001 = Set reference to power up in 40ms when analog blocks are powered up
    aic_writePage(1, 0x7C,
                  6); // Reg 124, Val = 6 = 0b00000110 = Charge Pump, full peak current (000), clock divider (110) to Div 6 = 333 kHz
    ...
    aic_writePage(1, 0x7D,
                  0x53); // Reg 125, Val = 0x53 = 0b01010011 = 0 10 1 00 11: HPL is master gain, Enable ground-centered mode, 100% output power, DC offset correction  // from WHF/CHA
